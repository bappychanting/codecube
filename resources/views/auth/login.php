...
<div class="card">
  ...
  <div class="card-body">
    <?php if(!empty(Base\Request::getData('timeout'))){ ?>
      <div class="alert alert-danger"><?= Base\Request::getData('timeout') ?></div>
      <h5 class="card-title">Spam Protection</h5>
      <p class="text-secondary"><?= Base\Request::show('captcha')['number1'].' + '. Base\Request::show('captcha')['number2'].' = ?' ?></p>
      <a href="<?= $_SERVER['REQUEST_URI'] ?>" class="btn btn-block bg-red waves-effect"><span class="oi oi-reload pr-2"></span> <span class="icon-name">ReCaptcha</span></a>
